...
class Token {
	...
	/**
	 * Gerenate the array of valid tokens to check for. These include two days
	 ...
	 * and wpforms_token_validity_in_days to extend the validation period). By default
	 * tokens are valid for day.
	 *
	 * @since 1.6.2
	 *
	...
	/**
	 * Add the  the token to the form attributes.
	 *
	 * @since 1.6.2
	 *
	...
	public function validate( array $form_data, array $fields, array $entry ) {
		...
		if ( ! isset( $entry['token'] ) ) {
			return $this->process_antispam_filter_wrapper( esc_html__( 'Form token is missing', 'wpforms-lite' ), $fields, $entry, $form_data );
		}

		// Verify the token.
		if ( ! $this->verify( $entry['token'] ) ) {
			return $this->process_antispam_filter_wrapper( esc_html__( 'Form token expired. Please refresh the page.', 'wpforms-lite' ), $fields, $entry, $form_data );
		}
