...
/**
 * Get the HTTP referer header value for an AJAX request, and confirm it has a
 * valid host.
 *
 * @since 1.6.2
 *
 * @return string|false The HTTP referer header value, or false on error.
 */
function wpforms_ajax_referer() {
	...
	$path = wp_parse_url( $ref, PHP_URL_HOST );
	$wpp  = wp_parse_url( home_url(), PHP_URL_HOST );

	// If referer host is different than home_url's host, then return false.
	if ( false === strpos( $wpp, $path ) ) {
		return false;
	}
